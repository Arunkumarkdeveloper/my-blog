...
export default function PostEditor({
  ...
  let keyTag: any;
    ...
    if (_tags[htmlTags]?._openTag === `<p>`) {
      keyTag = `<p class=${_space[space]}>${input1}</p>`;
    } else if (_tags[htmlTags]?._openTag === `<b>`) {
      keyTag = `<b class=${_space[space]}>${input1}</b>`;
    } else if (_tags[htmlTags]?._openTag === `<u>`) {
      keyTag = `<u class=${_space[space]}>${input1}</u>`;
    } else if (_tags[htmlTags]?._openTag === `<i>`) {
      keyTag = `<i class=${_space[space]}>${input1}</i>`;
    } else if (_tags[htmlTags]?._openTag === `<h1>`) {
      keyTag = `<h1 class=${_space[space]}>${input1}</h1>`;
    } else if (_tags[htmlTags]?._openTag === `<h2>`) {
      keyTag = `<h2 class=${_space[space]}>${input1}</h2>`;
    } else if (_tags[htmlTags]?._openTag === `<ul><li>`) {
      keyTag = `<ul class=${_space[space]} ><li>${input1}</li></ul>`;
    } else if (_tags[htmlTags]?._openTag === `<button>`) {
      keyTag = `<div class="d-flex justify-content-center" ><a href="" target="_blank" ><button class=${_space[space]} >${input1}</button></a></div>`;
    } else if (_tags[htmlTags]?._openTag === `<img>`) {
      keyTag = `<div class="d-flex justify-content-center" ><a href="" target="_blank" ><img src="${input1}" alt="${postData?.blogTitle}" title="${postData?.blogTitle}" class=${_space[space]} /></a></div>`;
    }
